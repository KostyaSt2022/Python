...
async def mes_new_game(message: types.Message):
    ...
    if first:
        await message.answer(f'Игра началась. По жребию первым ходит {message.from_user.first_name}! Бери конфеты от 1 до 28')
    else:
...
async def mes_duel(message: types.Message):
    ...
    first = random.randint(0,1)
    if first:
        await dp.bot.send_message(duel[0], 'Первый ход за тобой, бери конфеты (от 1 до 28)')
...
async def bot_turn(message: types.Message):
    ...
    if 0 < total < 29:
        bot_take = total
        total -= bot_take
        await message.answer(f'Банди взял {bot_take} конфет. '
                             f'На столе осталось {total} и Банди одержал победу')
        new_game = False
    else:
        remainder = total%29
